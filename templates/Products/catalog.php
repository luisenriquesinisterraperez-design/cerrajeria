...
$phone = $whatsappPhone ?? '573001234567';
?>
<header class="mb-8 text-center">
    <h1 class="text-3xl font-black text-slate-800 tracking-tight uppercase">Catálogo de Productos</h1>
    <p class="text-blue-600 font-bold uppercase text-xs tracking-widest">Seleccioná y solicitá tu pedido por WhatsApp</p>
</header>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-lg transition-shadow">
        ...
        <div class="p-5">
            ...
            <div class="space-y-2">
                <a href="https://wa.me/<?= $phone ?>?text=<?= urlencode("Hola, quiero comprar: {$product->name} - $$product->price") ?>"
                   target="_blank"
                   class="block w-full text-center py-3 bg-green-600 text-white rounded-xl font-black text-xs uppercase hover:bg-green-700 transition-all">
                    <i class="fa-brands fa-whatsapp mr-2"></i> Comprar
                </a>
                <a href="https://wa.me/<?= $phone ?>?text=<?= urlencode("Hola, quiero solicitar a crédito: {$product->name} - $$product->price") ?>"
                   target="_blank"
                   class="block w-full text-center py-3 bg-blue-600 text-white rounded-xl font-black text-xs uppercase hover:bg-blue-700 transition-all">
...
<div class="mt-10 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm text-center">
    <p class="text-xs text-slate-500 font-bold">
        <i class="fa-solid fa-shield-halved text-green-500 mr-1"></i>
        Pedidos seguros vía WhatsApp. Consultá por métodos de pago y envíos.
    </p>
</div>
